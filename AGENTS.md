...
App startup
  App.xaml.cs
    ...
    └─ MainWindow
         ...
         ├─ AudioVisualEngine → AudioVisualizerSurface (canonical local-audio simulation)
         │    └─ resolved primitive frame → ElyAudioCoreInterop → native D3D11 scene
...
- `OnboardingWindow.xaml(.cs)`: first-run wizard (6 steps). Collects the interface language (`en`/`fr`), display name, preferred connection (`xtream`/`m3u`/`local`) **with inline credentials/playlist and a live connection test**, accent colour and content interests. Language changes apply immediately in the wizard but, like all other onboarding choices, are persisted only on finish/skip. Step 5 maps the selected interests to an ELYSMART workload, runs the same asynchronous/cancellable `BenchmarkEngine` used by Settings, displays its measured score and explanations, and uses its proposed configuration to prefill editable renderer/VSR/FRUC choices. Step 6 downloads libmpv/shaders and runs `RtxVsrTester` when the chosen pipeline uses VSR. Credentials become a normal DPAPI profile plus `Settings.AutoConnectProfile`: MainWindow connects with it automatically at startup (cleared if the profile is deleted). `local` mode, and the "Continue without IPTV" button on the login screen, enter the local library directly without an IPTV login; selecting Live/Movies/Series from that shell returns to login instead of caching a false empty remote catalogue.
...
- `MainWindow.Catalog.cs`: navigation and media-library presentation. Owns `Nav_Changed`, lazy Movies/Series acquisition, list/filter/count rebuilding, local-library mutations, favourites, series drill-down and local-audio visualizer coordination. Single-track and grouped album/artist/genre/playlist removals share the batch local-library cleanup path so favourites, playlists, queue and playback context cannot retain removed tracks; removal never deletes the files on disk. The audio scene resolves the persisted background mode (`solid`, embedded `cover`, or one of `Assets/AudioBackgrounds`), extracts a small dominant-colour palette from the effective bitmap, applies blur/dimming and drives slow zoom/pan/optional mouse parallax. Navigation is keyed from the checked button's `Tag`; `NavSettings` must have only `Checked="Nav_Changed"` (never `Unchecked`) or WPF creates a second, racing navigation. Failed lazy loads leave `_movieItems`/`_seriesItems` null so the next click retries instead of caching a false empty result.
...
- `Services/LocalizationService.cs` / `LocalizationCatalog.cs`: English is the source language and default (`Settings.Language = "en"`); French is the complete secondary catalog. The service applies `en-US` or `fr-FR` UI culture before onboarding, registers localizable WPF text/content/header/title/tooltip/tag properties, and updates registered controls without rebuilding windows or playback backends. Dynamic/formatted UI text must use `LocalizationService.T` or `LocalizationService.Format`. Language selection is persisted through `StateStore`, exposed in onboarding and Settings > Interface, and must remain an immediate in-process switch. Do not add hardcoded French outside `LocalizationCatalog.cs`, and do not use em dashes in either language.
...
- `AudioVisualizerSurface.cs`: high-cadence WPF `OnRender` scene for FFT bars, particles and shockwaves. It accepts the effective image palette plus runtime particle count/distance; keep bitmap decoding/palette extraction outside this render surface. It is also the **canonical animation simulation for AudioCore+**: `CopyResolvedPrimitives` exposes the same already-resolved 112 bars, up to 192 particles and up to 6 shockwaves that `OnRender` draws, including positions, WPF-quantized colours/alpha and pen widths. `OnRender` itself consumes that resolved frame, so the classic and native paths must never duplicate geometry/style formulas or maintain separate random/beat simulations. Fixed bar trigonometry is precomputed, layout scale changes only on `SizeChanged`, and palette styles use bounded direct-array caches (32 hues × 9 alpha × 10 thickness × cap style), never per-frame dictionaries or unbounded caches. `AverageRenderTimeMs` is the measured CPU submission time shown in audio stats; 240 FPS requires comfortably below 4.17 ms. Palette extraction treats artwork with less than 12% meaningfully chromatic samples as monochrome and derives silver/gray luminance accents; the surface must not force saturation onto an achromatic palette. The target FPS, VSync, shake and background animation/parallax intensity policy are owned by `MainWindow.Catalog.cs`, with persisted controls in the `audio-player` settings category. `CompositionTarget.Rendering` is always physically compositor-bound: VSync ON uses a phase accumulator to hit arbitrary targets exactly (notably 240 on 360 Hz); VSync OFF unlocks updates up to the compositor refresh. The audio stats panel reports the measured accepted-frame rate from a rolling window, alongside target FPS, VSync, FFT publication rate and render CPU time; never label the configured target as real FPS. The packaged-background selector stays enabled in every mode because it also configures Cover mode's fallback when metadata has no embedded art. Both `_audioLastTickSeconds` and `_audioLastRenderedSeconds` must reset whenever its per-track stopwatch restarts; retaining the FPS-gate timestamp across tracks freezes rendering until the new stopwatch catches the previous track's elapsed time. Reset phase/player-sync/FPS counters there too. The full-screen blurred image uses `BitmapCache` at reduced render scale: animate only its cached transform, never force a Gaussian blur rerender each frame.
- Audio renderer selection is persisted as `Settings.AudioVisualizerRenderer`: `classic` draws the canonical frame through WPF; `audiocore` keeps `AudioVisualizerSurface` `Hidden` (laid out, no WPF submission) and sends that exact frame through `ElyAudioCoreInterop` to `ElyAudioCoreScene`. AudioCore+ is gated on ELYCORE, bypasses FRUC while active and falls back visibly to WPF when unavailable. The native D3D11 path draws background → particles → shockwaves → bars using source-over blending, reproduces the radial solid background or the WPF image pipeline (`UniformToFill`, 54-DIP overscan, 0.45 cache scale, separable Gaussian blur, 8-bit dimmer alpha, shared zoom/pan/parallax), and leaves the centre artwork/rings/title/metadata in the common WPF overlay. Every audio-player customization must affect both renderers through this shared state; do not add native-only animation policy.
...
- `Models/Settings.cs`: all global feature flags and ELYCOLOR/ELYSOUND+ profiles. Adding a setting requires normalization/default handling and UI load/save wiring. `Language` is normalized to `en` or `fr`, with `en` as the default. Local audio persists its sidebar grouping in `AudioBrowseMode`. ELYSOUND+ profile schema v2 stores a real `LimiterCeilingDb`; `StateStore.Normalize` migrates the legacy linear `Limiter` percentage and clamps every DSP control. Onboarding fields: `OnboardingCompleted`, `UserDisplayName`, `PreferredConnection`, `ContentInterests`, `Language`. `StateStore.Normalize` strips `fsr` from persisted `OsdUpscaleModes` (removed from OSD defaults).
...
- `NotificationEngine.cs`: cooldown/ignore policy, at most one issue emitted per evaluation. `MainWindow.ElySmart.cs` presents issues through one non-activating bottom-right `ElySmartNotificationWindow` toast with Optimize/Ignore/Always-ignore actions; toast presentation can be disabled with the persisted `Settings.ElySmartNotificationsEnabled` switch without stopping monitoring or automatic decorative optimization. Always-ignore kinds persist in `Settings.ElySmartIgnoredHealthIssues` and are restored at startup.
...
- `ElyFlowRendererInterop`: dynamically loads `ElyFlow.Native.dll`, requires the exact renderer ABI version before backend creation, validates every required export and mirrors the native state struct. ABI v10 reports VSR requested/available/effective state, processor/extension state, adapter/driver identity, DXGI formats/colour space, VSR processed/bypassed frames, Present errors, target rebuilds and real `ResizeBuffers` calls, and carries the AudioCore+ resolved-frame contract. Managed/native struct layout changes must be made on both sides together.
...
- `ElyAudioCoreScene.*`: AudioCore+ D3D11 background cache, analytic vector rasterization and resolved-frame storage. It consumes managed primitives; it does not own FFT, beat detection or visual simulation.
...
- `include/ElyFlowNative.h` / `include/ElyAudioCoreNative.h`: public video and AudioCore+ ABI shared conceptually with the managed interop structs.
...
AudioCore+ invariants:

- The WPF `CompositionTarget.Rendering` phase accumulator is the cadence authority. Each resolved-frame publication wakes the native thread; AudioCore+ must not manufacture duplicate animation frames from its own timer. `Present(1)` is used only when the persisted audio VSync setting is on; VSync off uses `Present(0)` while remaining bounded by managed compositor publications.
- Native primitives are rasterized from the managed normalized coordinates and packed AARRGGBB styles. Preserve the classic order (particles, shockwaves, bars), round caps for bars, WPF pen quantization and source-over blending. Never reduce the 192-particle contract or substitute one per-pixel palette colour for each primitive's resolved colour.
- The background image cache is rebuilt only for source, blur, DPI/root-size or target-size changes. Zoom, pan and parallax transform the cached result without rerunning the Gaussian passes. Resize/fullscreen rebuilds stay on the native render thread.
- WPF remains responsible for centre artwork, decorative rings, title and metadata panel in both modes. `AudioSurface` must be `Hidden`, not `Collapsed`, while AudioCore+ is active so its layout and canonical simulation remain valid.
...
- **NVCP "Super Resolution - Status" ignores VSR sessions whose input texture is written by another VideoProcessor.** `--watch-chain` (conversion VP writes the VSR VP's input) shows "Inactive" while the query reports in-use 0x4F; every other input source shows "Active": CPU-uploaded (`--watch`), graphics-pipeline-written RTV clears on the NV12 planes (`--watch-renderchain`), cross-device with WGL-interop + present (`--watch-cross-interop`), with a resident CUDA context (`--watch-cuda`). ELYCORE therefore converts RGB→NV12 with two pixel-shader draws by default; the former conversion VP remains as a fallback and can be forced with `ELYFLOW_VSR_CONV_VP=1` for A/B diagnostics. The indicator is cosmetic either way; `vsrQueryRaw`/pixel proof remain the ground truth.
...
- The OSD transport is responsive: wide players keep left/center/right controls on one row; below the compact-width threshold, the centered previous/play/next group takes its own row so it never overlaps either side.
...
- `LocalLibraryService` exclusively owns recursive extension-filtered folder discovery, TagLib enrichment, deduplication, deterministic artist/album/disc/track ordering and playlist path resolution. Startup displays persisted audio metadata immediately and refreshes it off the UI thread. Local favourites mirror audio metadata; a missing absolute path is repaired only from one unique filename match in the current library. `MainWindow.LocalMedia.cs` owns the WPF commands and session queue/shuffle/repeat state. Local audio has two distinct queue layers: starting a track establishes an invisible automatic continuation from the following tracks in the displayed order, while explicit user-enqueued tracks remain visible/countable and always play first in their insertion order; after the manual layer drains, automatic continuation resumes from its preserved cursor. Audio and video use distinct navigation sections and persisted collections; playlist/queue commands must never be exposed for local video.
...
For audio-renderer A/B validation on a clean state, use a local audio file and select the renderer transiently (the diagnostic override is not persisted and bypasses onboarding):

```powershell
$env:ELYCAST_DIAGNOSTIC_FILE = 'C:\absolute\path\audio.wav'
$env:ELYCAST_DIAGNOSTIC_AUDIO_RENDERER = 'classic'   # or 'audiocore'
& '.\artifacts\Release\ElyCast.exe'
```

This enters the player and logs renderer stats after five seconds. `DebugConsole` must tolerate environments where a Win32 console cannot be allocated so automated visual runs still start and retain file logging. For renderer changes validate: native and managed builds, real playback, focus churn, resize/fullscreen, OSD hover/click, VSR/FRUC toggles, `Present` errors, late presents and dropped frames. For AudioCore+ also compare both modes at the same window size/background/metadata layout, exercise every audio customization, verify the native status in the stats panel, and confirm FRUC stays bypassed.
